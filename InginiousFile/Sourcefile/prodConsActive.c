...
void* producteur(void* arg) {
    ...
    while (true) {
        ...
        semaphore_wait(&empty);  // Attente jusqu'à ce qu'il y ait de la place
        ...
        semaphore_post(&full);  // Signal qu'un nouvel élément est disponible
...
void* consommateur(void* arg) {
    while (true) {
        ...
        semaphore_wait(&full);  // Attente jusqu'à ce qu'un élément soit disponible
        ...
        verrou_unlock(&mutex);  // Libération du verrou
        semaphore_post(&empty);  // Signal qu'un espace est maintenant libre
